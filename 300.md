<a id="readme-top"></a>
# GPT-4o Image Prompts · 201-341

🎉 欢迎来到 GPT‑4o 图片提示词（Prompts）201-341 案例合集！

这里收录了最新的 201-341 号案例，结构与主仓保持一致，可单独查阅或配合脚本生成 JSON 数据集。
...
**提示词：**
```
Gemini Nano Banana — Transform the supplied portrait photo into a hyper-realistic 3D illustration. Rebuild the subject as a lifelike stylized character while preserving the exact face geometry, black frame glasses, hairstyle, and denim jacket. Keep the pose leaning forward with both hands gripping a glowing orange neon rectangular frame floating in front of the subject. Render a dark studio background with soft volumetric rim light and warm key light emitted by the neon frame. Emphasize clean skin shading, subsurface scattering, glossy eyewear reflections, and high-frequency fabric detail. Add subtle depth of field, high dynamic range highlights, and a cinematic mood. Output two square 4K renders: Version A faithful to the uploaded subject, Version B reimagined as an older charismatic entrepreneur while retaining wardrobe, pose, and lighting. Label each version in the corner with thin white sans-serif text.
```

**中文提示词：**
```
使用 Gemini Nano Banana 将提供的肖像照转换为超写实 3D 插画。重建人物为逼真又具风格化的角色，同时保持原有脸部结构、黑框眼镜、发型与牛仔外套。姿势维持身体前倾，双手抓住漂浮在身前的发光橙色霓虹矩形框。渲染深色棚拍背景，利用霓虹框提供的暖色主光和柔和体积轮廓光。强调干净皮肤质感、次表面散射、镜面眼镜反光与织物微细节。加入浅景深、高动态范围高光与电影感氛围。输出两张方形 4K 渲染：版本 A 贴合原始人物，版本 B 将其重塑为年长的魅力企业家，同时保留服装、姿势与灯光。为每个版本在角落添加细白无衬线文字标签。
```
